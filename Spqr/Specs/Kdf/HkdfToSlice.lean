...
/-- **Spec axiom for `spqr::kdf::hkdf_to_slice`**
• The Rust `hkdf` crate implements RFC 5869 HKDF-SHA256, so the octets written into `okm` are
  `hkdf salt ikm info okm.length`.
• Determinism in the arguments was already implied by `opaque`; the added assumption is that the
  output is independent of the prior contents of `okm` and agrees with the RFC.
