...
- **Gates**: five checks — `lake build` with no non-sorry warning, `lake exe runLinter Spqr`, `lake env lean scripts/Audit.lean`, `#print axioms` against the allowlist, and the provenance check. `#check_no_sorry` in `docs/ISSUE_TEMPLATE.md` does not exist in this repo and is removed in Phase 1, where the template becomes a per-property **PR** checklist
